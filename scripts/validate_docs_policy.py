...
# Do not allow more than 2 stories on the same subject in any trailing 4-day window.
...
def _story_subject(path: Path) -> str | None:
...
def main() -> int:
    ...
    # Enforce subject hard cap on newly added/modified story markdown files.
    changed_story_md = [
    ...
    for rel_path in changed_story_md:
        ...
        for candidate in DOCS.glob("*.md"):
            ...
            if cdt < window_start or cdt > story_dt:
                continue
            if _story_subject(candidate) == subject:
                subject_count += 1
                subject_names.append(candidate.name)
        if subject_count > 2:
            bad.append(
                f"Subject cap exceeded for '{subject}' ({subject_count} in trailing 4-day window for {rel_path.name}): {', '.join(sorted(subject_names))}"
            )
