...
def build_markdown(run_label: str, slot: str, candidates: int) -> str:
    ...
    lines.append("## Origin-diversity preflight (advisory, non-blocking)")
    lines.append("| claim_id | claim_criticality (high/med/low) | artifact_urls | origin_families_count | corroboration_minutes_spent | note | action |")
    lines.append("|---|---|---|---|---|---|---|")
    lines.append("| C1 |  |  |  |  |  | escalate/provisional/publish_with_uncertainty |")
    lines.append("")
    lines.append("Guidance:")
    lines.append("- Prefer independent origin corroboration for high-impact claims when feasible in-slot")
    lines.append("- If key claims remain single-origin, use explicit provisional language and continue if otherwise defensible")
    lines.append("- If contradiction is unresolved in-slot, publish with explicit uncertainty unless evidence is unusable")
    lines.append("")
