...
- [NOAA SWPC Alerts JSON](https://services.swpc.noaa.gov/products/alerts.json) — Machine-readable alerts, watches, and warnings for geomagnetic storms, radio blackouts, and solar radiation events; best first-stop source for operational status changes with explicit issue timestamps. High-confidence official feed, but message cadence can be bursty during active periods.
- [NOAA Planetary K-index Forecast JSON](https://services.swpc.noaa.gov/products/noaa-planetary-k-index-forecast.json) — Combined observed, estimated, and predicted 3-hour Kp values with NOAA storm-scale tags used to map likely disruption windows. Strong short-horizon planning signal with forecast-revision risk near event onset.
- [NOAA Planetary K-index Observed JSON](https://services.swpc.noaa.gov/products/noaa-planetary-k-index.json) — Observed planetary Kp intervals with running geomagnetic activity index and contributing station count for post-event verification. Useful measured baseline with normal measurement-latency caveats.
- [NOAA SWPC Solar Wind Plasma 7-day JSON](https://services.swpc.noaa.gov/products/solar-wind/plasma-7-day.json) — Near-real-time solar wind density, speed, and temperature series that help test whether upstream solar-wind conditions support or weaken storm expectations. Valuable mechanism layer with telemetry gaps and short-term noise.
