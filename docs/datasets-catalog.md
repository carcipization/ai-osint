...
- [NOAA SWPC Alerts JSON](https://services.swpc.noaa.gov/products/alerts.json) — Machine-readable alerts, watches, and warnings for geomagnetic storms, solar radiation storms, and radio blackouts with issue timestamps and supersession notes. Strong first-stop operational feed, with bursty cadence during active periods.
- [NOAA Scales JSON](https://services.swpc.noaa.gov/products/noaa-scales.json) — Current and forecast NOAA R/S/G scale levels (plus probability fields) for rapid severity-state checks without scraping narrative pages. Useful dashboard-grade signal with forecast uncertainty near event onset.
- [NOAA Solar Wind Magnetic Field 7-day JSON](https://services.swpc.noaa.gov/products/solar-wind/mag-7-day.json) — Near-real-time interplanetary magnetic field components (including Bz and total field) for testing geoeffective coupling conditions. Valuable mechanism layer with high-frequency noise and occasional telemetry gaps.
- [NOAA Solar Wind Plasma 7-day JSON](https://services.swpc.noaa.gov/products/solar-wind/plasma-7-day.json) — Near-real-time solar wind density, speed, and temperature stream used to contextualize expected geomagnetic response. Strong corroboration source for mechanism checks, but not direct impact proof on its own.
